...
def trainingParameters(in_put, hidden, out_put, learning_rate, momentum, batch_size, epochs):
    Layers = [in_put, hidden, out_put]
    model = Net(Layers)
    learning_rate = learning_rate
    optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate, momentum=momentum)
    train_loader = DataLoader(dataset=data_set, batch_size=batch_size)
    criterion = nn.CrossEntropyLoss()
    Results[f"Momentum %0.1f" % momentum] = train(data_set, model, criterion, train_loader, optimizer, epochs=epochs)
...
batch_size = 20
epochs = 100
trainingParameters(in_put, hidden, out_put, learning_rate, momentum, batch_size, epochs)
